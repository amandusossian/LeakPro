...
class TABDataset(Dataset):
    ...
    def subset(self, indices):
        return TABDataset(self.x[indices], self.y[indices], self.dec_to_onehot, self.one_hot_encoded)
...
def preprocess_tab_dataset(path):
    """Get the dataset, download it if necessary, and store it."""
    
    if os.path.exists(os.path.join(path, "tab_data.pkl")):
        with open(os.path.join(path, "tab_data.pkl"), "rb") as f:
