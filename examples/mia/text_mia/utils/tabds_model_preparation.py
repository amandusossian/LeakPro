...
from torch import device, optim, cuda, no_grad, hub, save, sigmoid
import pickle
from tqdm import tqdm

class BERTNER(nn.Module):
    def __init__(self, input_size=0, hidden_size=0, num_classes=0):
        super(BERTNER, self).__init__()
        # Based on this link https://pytorch.org/hub/huggingface_pytorch-transformers/
        self.tokenizer = hub.load('huggingface/pytorch-transformers', 'tokenizer', 'bert-base-uncased')    # Download vocabulary from S3 and cache.
        self.model = hub.load('huggingface/pytorch-transformers', 'model', 'bert-base-uncased')    # Download model and configuration from S3 and cache.
        
    
    def forward(self, x):
        out = self.model(self.tokenizer(x)) 
        return out
